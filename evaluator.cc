...
bool is_ugtsa_better(TensorflowWrapper *tensorflow_wrapper, int ugtsa_player, int ucb_strength, int ugtsa_strength) {
    ...
    while (!game_state.IsFinal()) {
        ...
        if (game_state.player == -1) {
            ...
            auto ugtsa_algorithm = UGTSAAlgorithm(&game_state, time(0), 5, tensorflow_wrapper, true);
...
int main(int argc, char **argv) {
    ...
    while (is_ugtsa_better(&tensorflow_wrapper, 0, end, ugtsa_strength)) {
    ...
    while (begin < end) {
        ...
        middle = (begin + end) / 2;
        if (is_ugtsa_better(&tensorflow_wrapper, 0, middle, ugtsa_strength)) {
            begin = middle + 1;
        } else {
            end = middle;
        }
